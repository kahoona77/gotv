      ...
      if (cmd == "SEND") {
        ...
        for {
          ...
          log.Printf("transferred: %d", totalBytes)
           ...
           // write a chunk
          if _, err := fo.Write(inBuf); err != nil {
              panic(err)
          }
...
func makeOutBuffer(totalBytes int) []byte {
    ...
    bytes[3] = byte(uint64(totalBytes) & 0xFF)
    bytes[2] = byte(uint64((totalBytes) >> 8) & 0xFF)
    bytes[1] = byte(uint64((totalBytes) >> 16) & 0xFF)
    bytes[0] = byte(uint64((totalBytes) >> 24) & 0xFF)
